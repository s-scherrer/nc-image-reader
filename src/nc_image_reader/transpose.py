...
from nc_image_reader.utils import infer_chunks
...
def create_transposed_netcdf(
    ...
    for i in trange(num_blocks):
        ...
        temporary_chunks.append(tmp_chunks)

        if Path(outfname + f".tmp.{i}").exists():
            continue
        block = reader.read_block(
            start=timestamps[s], end=timestamps[e - 1]
        ).chunk(dict(zip(new_dim_names, tmp_chunks)))
        ...
        with warnings.catch_warnings():
            ...
            transposed_block.to_netcdf(
                outfname + f".tmp.{i}",
                encoding={reader.varname: {"chunksizes": tmp_chunks}},
                engine="h5netcdf",
            )
    ...
    for i in range(num_blocks):
        with warnings.catch_warnings():
            ...
            ds = xr.open_dataset(
                outfname + f".tmp.{i}",
                chunks=dict(zip(new_dim_names, temporary_chunks[i])),
                engine="h5netcdf",
            )
            datasets.append(ds)
    ...
    final = xr.concat(
        ...
        data_vars="minimal",
        coords="minimal",
    ).chunk(dict(zip(new_dim_names, chunks)))
